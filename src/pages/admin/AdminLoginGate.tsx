  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.token) {
        ...
        setError(res.message || 'Invalid admin credentials. Access restricted to authorized personnel.');
      }
    } catch (err: any) {
      setError('Authentication server error. Please verify server connection.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border border-[#E5E0D8] p-8 shadow-xl relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#C06C4D]/10 border border-[#C06C4D]/25 text-[#C06C4D] text-[10px] font-bold uppercase tracking-wider">
            ...
            <span>MongoDB Database Protected</span>
          </div>
        ...
        <form onSubmit={handleAdminLogin} className="space-y-4 text-xs">
          ...
          >
            <span>{loading ? 'Verifying with Database...' : 'Sign In to Studio Portal'}</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </form>

        <div className="p-3 bg-[#F9F6F0] border border-[#E5E0D8] rounded-xl text-[11px] text-[#6B7280] text-center">
          <p>🔒 Authenticating directly against MongoDB Atlas database users.</p>
        </div>
      </div>
    </div>
  );
